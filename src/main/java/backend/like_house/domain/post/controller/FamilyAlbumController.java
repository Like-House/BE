...
public class FamilyAlbumController {
    ...
    public ApiResponse<List<FamilyAlbumDTO.AlbumPhotoResponse>> viewFamilyAlbum(
            @RequestParam Long familySpaceId,
            @RequestParam LocalDate date,
            @RequestParam(required = false) List<Long> taggedUserIds
    ) {
        List<FamilyAlbumDTO.AlbumPhotoResponse> response = familyAlbumQueryService.viewFamilyAlbum(familySpaceId, date, taggedUserIds);
    ...
    public ApiResponse<FamilyAlbumDTO.PostPreviewResponse> getPostPreview(
            @PathVariable Long postId
    ) {
        FamilyAlbumDTO.PostPreviewResponse response = familyAlbumQueryService.getPostPreview(postId);
        return ApiResponse.onSuccess(response);
    }
}